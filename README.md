...
本项目具体可分为客户端、服务端和数据库三个部分，其中客户端使用JavaScript、HTML和CSS实现，部分组件来自vant，图标来自iconfont，服务端主要使用python的flask框架实现，数据库环境为XAMPP中的MySQL。本项目具有药物信息的查询、删改，购物车，库存管理，物流追踪、信息收集、医生信息查询等功能。

本项目默认MySQL的本地端口为3306，测试数据库中的所有涉及在线url的内容均暂存在阿里云的OSS内。本项目的所有功能在本地均经测试并可正常运行，如需上线，只需把服务端和数据库迁移至服务器，并更改前后端的相关静态数据（具体参照后文）即可。

## 客户端(medicineShop_frontend)

客户端是用户看到的界面，即小程序本体，小程序的大部分信息都是从服务端接收来的。在用户进入客户端时，会自动进行登录操作，向微信服务器发送本次会话的code码后获取用户的openid用于进行库存管理等操作。为了确保用户在其他页面时一定为登陆状态，程序在加载首页时同样会进行一次登陆操作。客户端存储的某些静态信息（如服务器的域名）存储在 `/api/base.js` 中。
...
此处展示用户头像，id和待收货、全部订单、医生信息和健康信息的跳转栏

### 全部订单及待收货页面

这两个页面结构相同，仅显示的内容不同，此处采取的策略是以单次购买的单个商品为单位展示订单信息，全部订单页面显示所有订单，待收货页面仅显示代发货和待收货的商品，这两个页面中点击标题和图片可以跳转至对应的详情页面，点击查看物流信息的提示可跳转至物流页面，标题上方是商品的物流状态和订单编号，订单编号为结算时的时间（精确到秒）

### 物流页面

此处的物流页面仅作展示，运营时可连接合作物流公司的api获取物流信息

### 医生信息

用户可在本页面按科室分类选择医生，点击卡片后跳转至相应的详情页，可获取所选医生的详细资料和联系方式

### 健康信息

用户可在本页填写信息，带填写完成后提交按钮方可点击，点击后用户填写的数据将会被发送至服务器，而后弹出确认窗口，待用户点击确认后返回首页。

## 服务端(medicineShop_backend)
...
注：本项目暂未编写可实现交互式查询或删改数据库的脚本，如需操作数据库，需要手动执行相应的SQL命令。

## 数据库

数据库用来存储本项目涉及到的大部分数据，如商品信息、商品列表等，本项目涉及的测试用数据库位于SQL文件夹下。下面将分别介绍各个表格的内容和含义
...
本表共有 `openid` 、 `orderInfo` 两个字段，其中主键为 `openid` 用来存储用户的openid，类型为varchar。 `orderInfo` 用来存储单个用户的订单信息，依然以json的形式储存，考虑到用户可能在不同时间对单一商品下多次订单，本字段中的键为用户某一笔订单的下单时间，以字符串形式储存，形式为（20240303112508，精确到秒），值为json对象，键为字符串类型的商品id，值为json对象，此对象中以键值对的形式存储商品数量和订单状态（如-1代表支付失败，0代表待支付，1代表支付成功待发货，2代表待收货，3代表订单完成），后续可将时间转为订单编号并将时间信息添加到二级json对象中，本字段的形式样例如下：

`{"20240303112508":{"1":{"amount":6, status:1}, "3":{"amount":2, "status":3}}, "20240303113120":{"1":{"amount":5, "status":-1}}}`

### doctorinfo

本表储存医生的相关信息，有 `id` ， `name` ， `gender` ， `age` ， `skill`， `avatar` ， `info` ， `contact` ， `tag` 九个字段。分别存储医生的id、姓名、性别、年龄、技能、头像地址、个人信息、联系方式、所属科室。

### info

本表存储用户提交的健康信息，有 `openid`， `家庭慢性病情况`， `过敏情况` ， `是否有孩子` 四个字段，分别存储表格中的对应信息。
